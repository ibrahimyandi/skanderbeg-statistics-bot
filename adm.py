...
def administrative(save):
    ...
    for i in data:
        if 'player' in data.get(i)[0] and 'real_development_ratio' in data.get(i)[0]:
            admList.append(Players(data.get(i)[0]['player'],data.get(i)[0]['countryName'],float(data.get(i)[0]['real_development_ratio'])))
        admList = sorted(admList, key=attrgetter('value'), reverse=True)
    ...
    for i in data:
        if 'player' in data.get(i)[0] and 'weighted_avg_monarch' in data.get(i)[0] and 'human' in data.get(i)[0]:
            admList.append(Players(data.get(i)[0]['player'],data.get(i)[0]['countryName'],float(data.get(i)[0]['weighted_avg_monarch']['s'])))
        admList = sorted(admList, key=attrgetter('value'), reverse=True)
