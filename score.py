...
def scored(save):
    response = requests.get('https://skanderbeg.pm/api.php?key=a6aeaf7782e8c5444b8e9f55cb5abc36&scope=getCountryData&save={0}&value=player;countryName;human;overall_strength;provinces;monthly_income;&format=json'.format(save))
    data = response.json()
    scoreList = []
    class Players:
    ...
    for i in data:
        if 'player' in data.get(i)[0] and 'monthly_income' in data.get(i)[0] and 'provinces' in data.get(i)[0]:
    ...
    for i in range(len(scoreList)):
        print(scoreList[i].country,scoreList[i].oStrength)
