...
def aScored(save):
    response = requests.get('https://skanderbeg.pm/api.php?key=a6aeaf7782e8c5444b8e9f55cb5abc36&scope=getCountryData&save={0}&value=player;countryName;human;overall_strength;monthly_income;&format=json'.format(save))
    data = response.json()
    scoreList = []
    class Players:
